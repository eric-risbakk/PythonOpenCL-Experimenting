...
# Ask the user to select a platform or device on the CLI
...
# Create the compute program from the source buffer, then build it.
program = cl.Program(context, kernel_source).build()

# Create a and b vectors, then fill with random float values.
h_a = numpy.random.rand(LENGTH).astype(numpy.float32)
h_b = numpy.random.rand(LENGTH).astype(numpy.float32)
# Create an empty vector c  (a+b) to be returned from the compute device.
...
# Wait for commands to finish before reading back.
queue.finish()
rtime = time() - rtime
...
for a, b, c in zip(h_a, h_b, h_c):
    ...
    # Compute the deviation of the expected and output result
    tmp -= c
